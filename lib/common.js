...
const RedisKeys = require('../classes/Redis/RedisKeys');
...
const restrictedRoutes = [
    ...
    {route: '/admin/customers', response: 'redirect'},
    {route: '/admin/settings/menu', response: 'redirect'}
];
...
exports.checkAccess = (req, res, next) => {
    ...
    if([false, 'false'].includes(req.session.user.isAdmin) && routeCheck){
...
exports.getImages = async (dir, req, res, callback) => {
    let product = await RedisUtils.getAllValueFromHash(RedisKeys.getProductDetailsRedisKey(dir));
    // loop files in /public/uploads/ pattern matching
...
exports.getPaymentConfig = () => {
    let paymentGateway = config.get('payment.paymentGateway');
    return config.get(`payment.${paymentGateway}`);
};
...
exports.getMenu = () => {
    let menuCollection = MangoUtils.getDbCollection(Enums.menuCollectionName);
...
exports.newMenu = (req, res) => {
    ...
    .then((menu) => {
        ...
        return menuCollection.updateOne({}, {$set: {items: menu.items}}, {upsert: true})
        .then(() => {
            return true;
        });
    })
